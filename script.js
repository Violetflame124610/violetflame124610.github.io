    ...
    try {
        ...
        repos.forEach((repo, index) => {
            ...
            card.innerHTML = `
                ...
                <a href="project-detail.html?repo=${encodeURIComponent(repo.name)}" class="project-link">View Details →</a>
            `;

            card.style.cursor = 'pointer';
            card.addEventListener('click', (e) => {
                if (!e.target.closest('.project-link')) {
                    window.location.href = `project-detail.html?repo=${encodeURIComponent(repo.name)}`;
                }
            });
